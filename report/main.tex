...
\begin{abstract}
A brief overview of the preformance of two versions of binary search algorithms written in RISC-V assembly: one designed for a single-cycle processor and one designed for pipelining.
\end{abstract}
...
\IEEEPARstart{I}{n} this paper the performance characteristics of two version of a binary search algorithm written in RISC-V assembly will be discussed.

\section{Calculations}

First, various performance factors are measured from the Ripes simulation software and the others are calculated. The measured performance metrics are shown in Figure~\ref{1perf} for the single-cycle version and Figure~\ref{pipeperf} for the pipelined version.
...
\begin{comment}
se these observations to calculate:
– Total execution time
– Cycles per instruction (CPI)
– Speedup of pipelined version over non-pipelined
\end{comment}
